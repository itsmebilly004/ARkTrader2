...
export function lastDigitFromPrice(price: number, pipSize = 4) {
  if (!Number.isFinite(price)) return null;
  const text = price.toFixed(pipSize);
  const digit = Number(text.slice(-1));
  return Number.isInteger(digit) && digit >= 0 && digit <= 9 ? digit : null;
}

export function digitsFromPrices(prices: number[], pipSize = 4, limit?: number) {
  const digits = prices.map(p => lastDigitFromPrice(p, pipSize)).filter((digit): digit is number => digit != null);
